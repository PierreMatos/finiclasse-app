...
class SendPushNewLeadNotification
{
    ...
    public function handle(PushNewLead $event)
    {
        ...
        $vendors = User::where([['device_key', '!=', null]])->orwhere($event->user->vendor->first()->id)->pluck('device_key')->get();
        ...
        //Notification
        $vendorsNotification = User::where($event->user->vendor->first()->id)->get();
